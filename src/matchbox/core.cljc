...
(defn connect
  ...
  ([url]
   #?(:clj (FirebaseApp/initializeApp (-> (FirebaseOptions/builder)
                                          (.setCredentials (GoogleCredentials/fromStream (io/input-stream "/Users/ron/.config/gapi/serviceAccountKey.json")))
                                          (.setDatabaseUrl url)
                                          (.build)))
